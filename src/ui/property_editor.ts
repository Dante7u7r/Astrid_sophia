...
export class PropertyEditor {
  ...
  public init() {
    ...
    this.propIdInput?.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        e.preventDefault();
        handleApply();
      }
    ...
    this.propValInput?.addEventListener("keydown", (e) => {
      if (e.key === "Enter") {
        e.preventDefault();
        handleApply();
      }
          ...
          if (selected.type === 'vsource' || selected.type === 'isource') {
            ...
            if (waveTypeSelect && waveAmpInput && waveFreqInput && waveOffsetInput && waveDutyInput) {
              ...
              if (selected.waveType === "dc") {
                selected.value = selected.offset !== 0 ? selected.offset : newVal;
              } else {
                selected.value = selected.amplitude !== 0 ? selected.amplitude : (selected.offset || newVal);
              }
