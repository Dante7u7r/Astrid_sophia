...
export function calculateAutoFitSettings(
  ...
  if (!nodeId || results.length === 0) {
    return { voltsPerDiv: 1.0, timeDivValue: 0.001, offsetPixels: 0 };
  }

  const metrics = calculateOscilloscopeMetrics(results, nodeId);
  const vpp = metrics.vpp > 0 ? metrics.vpp : 1.0;

  const standardVoltsDiv = [0.005, 0.01, 0.02, 0.05, 0.1, 0.2, 0.5, 1.0, 2.0, 5.0, 10.0, 20.0];
  let targetVoltsDiv = 1.0;
  for (const vdiv of standardVoltsDiv) {
    if (vdiv * 4 >= vpp) {
      targetVoltsDiv = vdiv;
      break;
    }
  ...
  const freq = metrics.freq;
  if (freq > 0) {
    const period = 1.0 / freq;
    const desiredTimeDiv = (period * 3.0) / 10.0;
    const standardTimeDiv = [
      1e-6, 2e-6, 5e-6, 1e-5, 2e-5, 5e-5,
      1e-4, 2e-4, 5e-4, 1e-3, 2e-3, 5e-3,
      1e-2, 2e-2, 5e-2, 0.1, 0.2, 0.5,
    ];
    let minDiff = Infinity;
    for (const tdiv of standardTimeDiv) {
      const diff = Math.abs(Math.log10(tdiv) - Math.log10(desiredTimeDiv));
      if (diff < minDiff) {
        minDiff = diff;
        targetTimeDiv = tdiv;
      }
    ...
    const totalDuration = results[results.length - 1].time - results[0].time;
    if (totalDuration > 0) {
      targetTimeDiv = totalDuration / 10.0;
    }
  }

  return {
    voltsPerDiv: targetVoltsDiv,
    timeDivValue: targetTimeDiv,
    offsetPixels: 0,
  };
}
