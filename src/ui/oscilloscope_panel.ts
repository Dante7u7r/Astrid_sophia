...
export class OscilloscopePanel {
  ...
  public draw() {
    // 1. Always step simulation playback time and notify listeners whenever simulation is active
    ...
    // 2. Always schedule next frame if simulation is active so continuous 60 FPS playback never stops
    this.scheduleNextFrame();

    // 3. If DOM canvas is not connected or hidden, skip drawing pixels
    if (!this.oscCanvas || !this.oscCtx || !this.isCanvasVisible()) return;
    ...
    if (this.activeAnalysisMode === 'AC' && this.acSweepResults !== null && this.acSweepResults.frequencies.length > 0) {
      ...
      // Calculate live sweep beam X position
      const windowDuration = this.timeDivValue * 10;
      const sweepRatio = windowDuration > 0 ? (this.sweepTime % windowDuration) / windowDuration : 1.0;
      const sweepX = sweepRatio * width;
      ...
      const drawChannelTY = (nodeId: string, color: string, voltsPerDiv: number, offsetPixels: number, isActive: boolean) => {
        ...
        for (let i = 0; i < tracePoints.length; i++) {
          ...
          if (this.isSimulating && point.x > sweepX) break;
        ...
        // Draw live sweep beam head glowing particle dot
        if (this.isSimulating && headPoint) {
      ...
      // Draw vertical CRT Sweep Beam line when simulating
      if (this.isSimulating && !this.isOscPaused) {
  ...
  public autoFit(channel: OscilloscopeChannel = "ch1"): void {
    const probeNode = this.getProbeNodeByChannel(channel);
    if (!probeNode || this.transientResults.length === 0) return;

    const fit = calculateAutoFitSettings(this.transientResults, probeNode);
    this.timeDivValue = fit.timeDivValue;
    if (this.timeDivSelect) {
      this.timeDivSelect.value = fit.timeDivValue.toString();
    }

    if (channel === "ch1") {
      this.voltsPerDivCh1 = fit.voltsPerDiv;
      this.offsetCh1 = fit.offsetPixels;
      if (this.voltsCh1Select) this.voltsCh1Select.value = fit.voltsPerDiv.toString();
      if (this.offsetCh1Slider) this.offsetCh1Slider.value = "0";
    } else if (channel === "ch2") {
      this.voltsPerDivCh2 = fit.voltsPerDiv;
      this.offsetCh2 = fit.offsetPixels;
      if (this.voltsCh2Select) this.voltsCh2Select.value = fit.voltsPerDiv.toString();
      if (this.offsetCh2Slider) this.offsetCh2Slider.value = "0";
    } else if (channel === "ch3") {
      this.voltsPerDivCh3 = fit.voltsPerDiv;
      this.offsetCh3 = fit.offsetPixels;
      if (this.voltsCh3Select) this.voltsCh3Select.value = fit.voltsPerDiv.toString();
      if (this.offsetCh3Slider) this.offsetCh3Slider.value = "0";
    } else if (channel === "ch4") {
      this.voltsPerDivCh4 = fit.voltsPerDiv;
      this.offsetCh4 = fit.offsetPixels;
      if (this.voltsCh4Select) this.voltsCh4Select.value = fit.voltsPerDiv.toString();
      if (this.offsetCh4Slider) this.offsetCh4Slider.value = "0";
    }
