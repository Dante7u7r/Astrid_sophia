...
function validateFallbackNetlist(netlist: CircuitNetlist): string | null {
  ...
  for (const comp of netlist.components) {
    ...
    ids.add(comp.id);
    const expectedPins = comp.type === "ground"
      ? 1
      : (comp.type === "nmos" || comp.type === "pmos" || comp.type === "npn" || comp.type === "pnp" || comp.type === "potentiometer")
        ? 3
        : comp.type === "opamp"
          ? 5
          : comp.type === "transformer"
            ? 4
            : comp.pins.length; // Allow custom pin counts for MCUs and complex ICs
