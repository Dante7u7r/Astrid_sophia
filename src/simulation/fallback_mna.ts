...
/**
 * Genera números aleatorios con distribución normal (Gaussiana)
 * mediante la transformada de Box-Muller para Monte Carlo.
 */
export function boxMullerRandom(mean: number = 0, stdDev: number = 1): number {
  let u1 = 0;
  let u2 = 0;
...
/**
 * Aplica variación estocástica de tolerancia a componentes RLC en TS Fallback.
 */
export function applyMonteCarloVariation(
  nominalValue: number,
  tolerancePercent: number,
): number {
  if (tolerancePercent <= 0) return nominalValue;
  const stdDev = (nominalValue * (tolerancePercent / 100)) / 3.0; // 3-sigma tolerance
  return Math.max(1e-15, boxMullerRandom(nominalValue, stdDev));
}
