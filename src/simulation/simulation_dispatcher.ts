      ...
      fallbackTimeoutId = setTimeout(async () => {
        ...
        if (rustOnlyLabel) {
          ...
          if (typeof tsRes === "string") {
            ...
            callbacks.onIpcStatusUpdate("Respaldo Transitorio local (Activo)", "var(--accent-cyan)");
            callbacks.updateCanvasRendering();
            // Simulación interactiva continua: permanece activa hasta que el usuario presione Detener.
          }
        } else {
