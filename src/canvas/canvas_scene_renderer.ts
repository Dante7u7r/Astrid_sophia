...
export class CanvasSceneRenderer {
  ...
  public render(_voltageMap: Record<string, number> = {}, probes: ProbeBadges = {}, nodeMap: Record<string, string> = {}, sparMarkers?: SParameterMarker[]): void {
    ...
    // 3. Draw Wires
    this.drawWires(componentById, pinCache, visibleWorldBounds, _voltageMap);
  ...
  private drawWires(
    ...
    for (const wire of this.host.wires) {
      ...
      // Draw animated current flow particles when simulation is active
      if (voltageMap && Object.keys(voltageMap).length > 0) {
        this.ctx.save();
      ...
      for (const pin of pins) {
        ...
        if (isHovered || isActive) {
          ...
          if (isHovered && nodeId) {
            ...
            // Draw Mini Hover Waveform Box when simulating
            if (volt !== undefined) {
              const miniW = 72;
              const miniH = 30;
              const miniX = pin.x - miniW / 2;
              const miniY = boxY - miniH - 4;

              this.ctx.fillStyle = "rgba(8, 12, 22, 0.95)";
              this.ctx.strokeStyle = "rgba(102, 252, 241, 0.6)";
              this.ctx.lineWidth = 1;
              this.ctx.beginPath();
              this.ctx.roundRect(miniX, miniY, miniW, miniH, 4);
              this.ctx.fill();
              this.ctx.stroke();
              ...
              this.ctx.beginPath();
              this.ctx.moveTo(miniX, miniY + miniH / 2);
              this.ctx.lineTo(miniX + miniW, miniY + miniH / 2);
              this.ctx.stroke();

              // Draw animated live waveform curve inside tooltip
              ...
              const ampScale = Math.min(Math.max(Math.abs(volt) / 10, 0.25), 1.0);
              for (let px = 2; px < miniW - 2; px += 2) {
                const phase = (px / miniW) * Math.PI * 4 - now;
                const waveY = miniY + miniH / 2 - Math.sin(phase) * (miniH * 0.35 * ampScale);
                if (px === 2) this.ctx.moveTo(miniX + px, waveY);
                else this.ctx.lineTo(miniX + px, waveY);
              }
              this.ctx.stroke();
              this.ctx.shadowBlur = 0;
            }
