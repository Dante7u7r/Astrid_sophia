...
export class SimulationController {
  ...
  async runSimulation(mode: AnalysisMode): Promise<void> {
    ...
    let activeMode = mode;
    if (activeMode === "DC") {
      const hasDynamicComponents = netlist.components.some(c => {
        const valObj = typeof c.value === "object" && c.value !== null ? (c.value as Record<string, any>) : {};
        const wave = (valObj.waveType || "").toString().toLowerCase();
        const freq = Number(valObj.frequency ?? 0);
        const type = c.type.toLowerCase();
        return (
          wave === "sine" || wave === "square" || wave === "pulse" || wave === "triangle" || wave === "sawtooth"
          || freq > 0
          || type.startsWith("mcu_") || type.startsWith("arduino_") || type.startsWith("esp32")
        );
      });
      if (hasDynamicComponents) {
        activeMode = "TRAN";
        this.setActiveAnalysisMode("TRAN");
        this.dependencies.addLog("⚡ [Auto-Detect] Detectadas fuentes oscilantes/MCUs; ajustado a Análisis Transitorio (TRAN).", "system");
      }
    }
    ...
    await dispatchSimulation(netlist, activeMode, {
