...
function bundleArgument(argv) {
  ...
  return process.env.BIAANI_SUPPORT_BUNDLE || process.env.ASTRYD_SUPPORT_BUNDLE;
}
...
function validateBundle(value) {
  ...
  if (value.manifest?.format !== "biaani-feedback-support" && value.manifest?.format !== "astryd-feedback-support") {
...
function loadBundle(pathValue) {
  if (!pathValue) throw new Error("Indica un paquete con --bundle <archivo.json> o BIAANI_SUPPORT_BUNDLE.");
...
function resources(bundle) {
  return [
    { uri: "biaani://support/summary", name: "summary", title: "Resumen de diagnóstico", mimeType: "text/markdown" },
    { uri: "biaani://support/manifest", name: "manifest", title: "Manifiesto y redacciones", mimeType: "application/json" },
    { uri: "biaani://support/events", name: "events", title: "Eventos redactados", mimeType: "application/json" },
    { uri: "astryd://support/summary", name: "summary", title: "Resumen de diagnóstico (legacy)", mimeType: "text/markdown" },
    { uri: "astryd://support/manifest", name: "manifest", title: "Manifiesto y redacciones (legacy)", mimeType: "application/json" },
    { uri: "astryd://support/events", name: "events", title: "Eventos redactados (legacy)", mimeType: "application/json" },
  ];
}
...
try {
  ...
  process.stderr.write(`Biaani MCP: ${error instanceof Error ? error.message : String(error)}\n`);
  process.exit(1);
}
...
input.on("line", (line) => {
  ...
  if (message.method === "initialize") {
    ...
    success(message.id, {
      ...
      serverInfo: {
        name: "biaani-feedback-readonly",
        title: "Biaani — diagnóstico local de solo lectura",
        version: APP_VERSION,
  ...
  if (message.method === "resources/list") {
    audit("resources/list", { resultCount: 6 });
  ...
  if (message.method === "resources/read") {
    ...
    let text;
    let mimeType;
    if (uri === "biaani://support/summary" || uri === "astryd://support/summary") {
      text = bundle.summaryMarkdown;
      mimeType = "text/markdown";
    } else if (uri === "biaani://support/manifest" || uri === "astryd://support/manifest") {
      text = JSON.stringify(bundle.manifest, null, 2);
      mimeType = "application/json";
    } else if (uri === "biaani://support/events" || uri === "astryd://support/events") {
