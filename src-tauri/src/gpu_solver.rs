...
fn estimate_condition_number(s: &DMatrix<f64>) -> f64 {
    let n = s.nrows();
    ...
    for r in 0..n {
        ...
        for c in 0..n {
            ...
            if r == c {
                ...
                if val > 0.0 && val < min_diag { min_diag = val; }
            }
        }
        if row_sum > norm_inf {
            norm_inf = row_sum;
    ...
    (norm_inf / min_diag).max(max_diag / min_diag)
}
...
pub fn solve_schur_on_gpu(s_matrix: &DMatrix<f64>, z: &DVector<f64>) -> Option<DVector<f64>> {
    ...
    // Si la matriz está mal condicionada, fallback automático a CPU SuperLU para preservar precisión IEEE-754 f64
    if estimate_condition_number(s_matrix) > 1e5 {
        return None;
    }
