...
use families::{classify_component, resolve_device_layout, resolve_pin_count};
pub use models::*;
...
pub fn parse_spice_netlist_to_native(netlist_str: &str) -> Result<CircuitNetlist, String> {
    ...
    for line in root_lines {
        ...
        let layout = resolve_device_layout(first_char, &tokens, &line, &models);
        let is_gate = layout.is_gate;
        let is_subckt = layout.is_subcircuit;
        ...
        if is_subckt {
            ...
            let pins_count = resolve_pin_count(&layout, first_char, &tokens, &models);
            ...
            let comp_type =
                classify_component(first_char, value_or_model, &tokens, is_gate, &models);
