...
pub(super) fn stamp_nmos(comp: &ComponentData, ctx: &mut StampContext<'_>) {
    ...
    let (ids, gm, gds, gmb, igs, gg) = if comp.comp_type == "igbt" {
        ...
        (res.ic, res.gm, res.go, 0.0, 0.0, 1e-12)
    } else if comp.comp_type == "sic_mosfet" {
        let params = SicMosfetParams {
            vth: if comp.value > 0.0 { comp.value } else { 2.5 },
            rds_on: comp.ron.unwrap_or(0.045),
            ..SicMosfetParams::default()
        };
        let res = evaluate_sic_mosfet(vgs, raw_vds, 300.0, &params);
        (res.ids, res.gm, res.gds, 0.0, 0.0, 1e-12)
        ...
        (ids_val, gm_val, gds_val.max(1e-9), 0.0, 0.0, 1e-12)
    } else {
        // Saturación
        let ids_val = kn * (vgs - vth) * (vgs - vth);
        let gm_val = 2.0 * kn * (vgs - vth);
        let gds_val = 1e-5;
        (ids_val, gm_val, gds_val, 0.0, 0.0, 1e-12)
    };

    let ieq = ids - gm * vgs - gds * vds - gmb * vbs;
    ...
    // Estampar transconductancia de sustrato/cuerpo gmb dependiente de Vb y Vs
    if gmb.abs() > 1e-15 {
        if node_drain > 0 {
            if node_bulk > 0 {
                matrix_a.add_element(node_drain - 1, node_bulk - 1, gmb);
            }
            if node_source > 0 {
                matrix_a.add_element(node_drain - 1, node_source - 1, -gmb);
            }
        }
        if node_source > 0 {
            if node_bulk > 0 {
                matrix_a.add_element(node_source - 1, node_bulk - 1, -gmb);
            }
            if node_source > 0 {
                matrix_a.add_element(node_source - 1, node_source - 1, gmb);
            }
        }
    }
...
pub(super) fn stamp_pmos(comp: &ComponentData, ctx: &mut StampContext<'_>) {
    ...
    let (isd, gm_sd, gds_cond, gmb_sd, igs, gg) = if comp.comp_type == "bsim4pmos" {
        evaluate_bsim4_pmos(
            vsg,
            vsd,
            vsb,
        ...
        (isd_val, gm_sd_val, gds_cond_val.max(1e-9), 0.0, 0.0, 1e-12)
    } else {
        let isd_val = kp * (vsg - vth_abs) * (vsg - vth_abs);
        let gm_sd_val = 2.0 * kp * (vsg - vth_abs);
        let gds_cond_val = 1e-5;
        (isd_val, gm_sd_val, gds_cond_val, 0.0, 0.0, 1e-12)
    };

    let ieq_sd = isd - gm_sd * vsg - gds_cond * vsd - gmb_sd * vsb;
    ...
    // Estampar transconductancia de sustrato/cuerpo gmb_sd para PMOS
    if gmb_sd.abs() > 1e-15 {
        if node_drain > 0 {
            if node_source > 0 {
                matrix_a.add_element(node_drain - 1, node_source - 1, -gmb_sd);
            }
            if node_bulk > 0 {
                matrix_a.add_element(node_drain - 1, node_bulk - 1, gmb_sd);
            }
        }
        if node_source > 0 {
            if node_source > 0 {
                matrix_a.add_element(node_source - 1, node_source - 1, gmb_sd);
            }
            if node_bulk > 0 {
                matrix_a.add_element(node_source - 1, node_bulk - 1, -gmb_sd);
            }
        }
    }
