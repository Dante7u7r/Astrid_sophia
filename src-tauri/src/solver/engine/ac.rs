        ...
        if comp.comp_type == "diode" || comp.comp_type == "led" {
            ...
            let (ids, gm, gds, igs, gg) = if comp.comp_type == "bsim4nmos" {
                ...
                if vgs <= vth {
                    ...
                    let gds_val: f64 = i_sub0 * exp_sub * ( (exp_vds / vt) * (1.0 + lambda * vds) + sub_factor * lambda );
                    ...
                    let gds_val: f64 = (2.0 * kn * (vgs - vth - vds)) * (1.0 + lambda * vds) + triode_curr * lambda;
                    (ids_val, gm_val, gds_val.max(1e-9), 0.0, 1e-12)
                } else {
